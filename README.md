...
Fyrypt's UID firewall (main) screen has 2 views (select from top right menu):
- **Apps**  
...
Fyrypt's process firewall screen has 3 views (select from top right menu):
...
Add apps, UIDs, processes and services to a group. And then block / unblock the whole group. Long press on an item to add/remove it to/from a group.
...
Firewall service notification shows a button to unblock the foreground app which is visible on screen (provided that it has INTERNET permission and is blocked). If you unblock the app, it remains unblocked as long as you are using it. After being closed, Fyrypt blocks the app after 30 seconds.
...
- Fyrypt supports user accounts, profiles, and Android 15 private space.
- For convenient access from home / launcher, app shortcuts are available to groups and logs.
- You can conveniently do backup / restore of settings and preferences.
...
- Despite having root privileges, Fyrypt does not make any persistent changes to your device. No files are created outside the app directories, no system components are touched. Disable services, ungrant root / ADB access, and uninstall the app. Even if something goes unexpected, do a device reboot (e.g. to remove `iptables` rules injected by Fyrypt). And all is clean. Do not forget to remove the boot script, if enabled in settings.

## Limitations of Fyrypt
- Requires root, or `adb root`, as stated above.
- PID firewall requires a custom kernel as explained above.
